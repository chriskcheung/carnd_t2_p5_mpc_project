...
size_t N = 10; //8
double dt = 0.1; // to match the latency of the actuation
...
// defines the beginning point of singular vector index where each 
// state variable starts
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    // TODO: implement MPC
    // `fg` a vector of the cost constraints (of the state vector), 
	...
	for (int i=0; i<int(N); i+=1){
		fg[0] += cnstr_weight_cte  * CppAD::pow(vars[cte_start+i]-ref_cte,   2);
		fg[0] += cnstr_weight_epsi * CppAD::pow(vars[epsi_start+i]-ref_epsi, 2);
		fg[0] += cnstr_weight_v    * CppAD::pow(vars[v_start+i]-ref_v,       2);
	}
	...
	for (int i=0; i<int(N)-2; i+=1){
		fg[0] += cnstr_weight_delta_gap * CppAD::pow(vars[delta_start+i+1]-vars[delta_start+i], 2);
		fg[0] += cnstr_weight_a_gap     * CppAD::pow(vars[a_start+i+1]-vars[a_start+i],         2);
	}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // element vectors (delta and a) and there are 10 timesteps. The number of variables is:
  // 6 * 10 + 2 * (10-1)
  size_t n_vars = state.size() * N + 2 * (N-1);
  // TODO: Set the number of constraints
  size_t n_states = N*state.size();
  ...
  // creates a 2 element double vector.s
